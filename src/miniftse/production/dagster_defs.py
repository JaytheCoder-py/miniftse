...
def build_job(securities: int = 200, seed: int = 20260809, simulate: str | None = None) -> DailyJob:
    return DailyJob(
...
if DAGSTER_AVAILABLE:  # pragma: no cover - requires the orchestration extra
    ...
    def validated_inputs(
        context: AssetExecutionContext, market_data: dict[str, Any]
    ) -> Output[dict[str, Any]]:
        job = build_job(simulate=context.run.tags.get("simulate"))
    ...
    def index_level(
        ...
        calc = job.calculate_index(
            {
                "run_date": run_date,
                "load_market_data": market_data["data"],
    ...
    def gate_decision(
        ...
        ctx = {
            ...
            "load_market_data": market_data["data"],
            "validate_inputs": validated_inputs,
            "calculate_index": index_level["calc"],
        }
    ...
    @asset(
        group_name="daily_production", description="Persist the new state and write a run manifest."
    )
    def published_index(
        context: AssetExecutionContext,
        market_data: dict[str, Any],
    ...
    defs = Definitions(
        assets=[market_data, validated_inputs, index_level, gate_decision, published_index],
...
def materialise(run_date: dt.date, simulate: str | None = None) -> dict[str, Any]:
    ...
    result = materialize(
        ...
        tags={"run_date": run_date.isoformat(), **({"simulate": simulate} if simulate else {})},
        raise_on_error=False,
    )
    materialised = (
        [
            e.node_name
            for e in result.events_for_node("published_index")
