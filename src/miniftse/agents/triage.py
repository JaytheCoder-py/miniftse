...
class TriageToolkit:
    ...
    def query_price_history(self, security_id: str, as_of: dt.date, days: int = 10) -> ToolResult:
        window = (
            self.prices[
        ...
        if window.empty:
            return ToolResult(
                ...
                None,
                f"no price history for {security_id}",
            )
        ...
        return ToolResult(
            ...
            (
                ...
                if moves.size
                else f"{len(window)} session"
            ),
        )

    def get_corporate_actions(
        self, security_id: str, as_of: dt.date, window: int = 10
    ) -> ToolResult:
        ...
        return ToolResult(
            ...
            hits,
            f"{len(hits)} event(s) within {window} days: {types}",
        )

    def compare_sources(self, security_id: str, as_of: dt.date) -> ToolResult:
        if self.alternate_prices is None:
            return ToolResult(
                "compare_sources", {"security_id": security_id}, None, "no second source configured"
            )
        primary = self.prices[
            (self.prices["security_id"] == security_id) & (self.prices["date"] == as_of)
        ]
        ...
        return ToolResult(
            "compare_sources",
            {"security_id": security_id, "as_of": str(as_of)},
    ...
    def get_index_weight(self, security_id: str) -> ToolResult:
        if self.weights is None:
            return ToolResult(
                "get_index_weight", {"security_id": security_id}, None, "no weights available"
            )
        ...
        return ToolResult(
            ...
            {"security_id": security_id},
            weight,
            f"index weight {weight:.4%}" + (" - immaterial" if weight < 0.0005 else ""),
        )

    def estimate_index_impact(self, security_id: str, price_move: float) -> ToolResult:
        weight = float(self.weights.get(security_id, 0.0)) if self.weights is not None else 0.0
        impact = weight * price_move
        return ToolResult(
            "estimate_index_impact",
            {"security_id": security_id, "price_move": price_move},
            impact,
            f"{impact * 10_000:+.2f}bp of index return",
        )

    def check_peers(self, security_id: str, as_of: dt.date, peers: list[str]) -> ToolResult:
        ...
        if not prior_dates:
            return ToolResult("check_peers", {"security_id": security_id}, None, "no prior session")
        prior = prior_dates[-1]
        ...
        subject = (
            float(today.get(security_id, np.nan)) / float(before.get(security_id, np.nan)) - 1.0
        )
        return ToolResult(
            "check_peers",
    ...
    def as_tools(self) -> dict[str, Tool]:
        return {
            ...
            "compare_sources": Tool(
                "compare_sources",
                "Primary feed against a second source - separates a bad feed from a real move.",
                self.compare_sources,
            ),
            "get_index_weight": Tool(
                "get_index_weight", "Index weight, for materiality.", self.get_index_weight
            ),
            "estimate_index_impact": Tool(
                "estimate_index_impact", "Index impact in basis points.", self.estimate_index_impact
            ),
            "check_peers": Tool(
                "check_peers", "Whether comparable securities moved the same way.", self.check_peers
            ),
        }
...
class TriageAgent:
    ...
    def triage(
        self, finding: Finding, as_of: dt.date, peers: list[str] | None = None
    ) -> TriageNote:
        ...
        if security:
            results.append(tools["query_price_history"](security_id=security, as_of=as_of))
            results.append(tools["get_corporate_actions"](security_id=security, as_of=as_of))
            results.append(tools["get_index_weight"](security_id=security))
            if peers:
                results.append(tools["check_peers"](security_id=security, as_of=as_of, peers=peers))
            if self.toolkit.alternate_prices is not None:
                results.append(tools["compare_sources"](security_id=security, as_of=as_of))
    ...
    def _rank(self, finding: Finding, results: list[ToolResult]) -> list[Hypothesis]:
        ...
        if weight is not None and isinstance(weight.output, float) and weight.output < 0.0005:
            out.append(
                Hypothesis(
    ...
    def _prompt(
        ...
        ranked = "\n".join(
            f"{i}. {h.cause} [{h.likelihood}]: {h.evidence}" for i, h in enumerate(hypotheses, 1)
        )
        return (
